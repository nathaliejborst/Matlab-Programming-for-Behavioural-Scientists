...
% Create figure with the y- and x-axis ranging between 0-1 (DO WE NEED TO
% DEFINE NAME??
stimuliFig = figure('Name','Stimuli', 'Color', [0 0 0])
xlim([0,1]);    % set x dimensions figure
ylim([0,1]);    % set y dimensions figure
...
if taskTable{'targetSymbol','distColor'}>0
    for i = 1:taskTable{'targetSymbol','distColor'} % Place the target symbol and distractor color combination-s in figure
        ...
        set(stimuliFig, 'Visible', 'off');  % hide figure
        hold on
    end
end
% Check if there are target symbol and distractor color combination-s
